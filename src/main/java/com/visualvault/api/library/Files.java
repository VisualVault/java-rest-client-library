...
public class Files extends BaseApi {

    public Files(Token token) {
        super(token);
    }
    ...
    // uploadFile uploads a file and attaches it to an existing Document as a new
    // file revision
    // OR replaces a file revision depending upon the CheckInState parameter value.
    //
    // documentId is the GUID/UUID (unique identifier) value for an existing
    // Document. Use the Documents endPoint to get a documentId using the Document
    // name)
    // revisionNumber is the revision number you wish to assign to the file i.e.
    // current document at revision 1 so I will upload the file at revision 2.
    // checkInDocumentState is either "Released" or "Unreleased" or "Replace"
    // (replace the existing revision's file).
    // if you want to update document index fields values you must provide a json
    // string representing key value pairs.
    // i.e. let indexFields = "{\"Customer Name\":\"ACME\",\"Favorite
    // Drink\":\"mountain dew\"}".
    // if the document has no index fields then pass an empty json document "{}" for
    // the index fields parameter.
    ...
    public String uploadFile(String documentId, String revision, String changeReason, String checkInDocumentState,
            String indexFields, String fileName, InputStream inputStream, String mimeType, String charset)
            throws Exception {
        return uploadFile(documentId, revision, changeReason, checkInDocumentState, indexFields, fileName, inputStream,
                mimeType, charset, "{}");
    }
    ...
    // mimeType is the mimeType of the file being uploaded.
    // charset is "UTF-8" in most cases.
    // if you want to optional parameters you must provide a json
    // string representing key value pairs.
    // i.e. let optionalParameters = "{\"Customer Name\":\"ACME\",\"Favorite
    // Drink\":\"mountain dew\"}".
    // if the document has no additional parameters then pass an empty json document "{}" for
    // the optional parameters argument.
    public String uploadFile(String documentId, String revision, String changeReason, String checkInDocumentState,
            String indexFields, String fileName, InputStream inputStream, String mimeType, String charset,
        ...
        try {
            ...
            writer.append("Content-Type: text/plain; charset=" + charset).append(CRLF);
            writer.append(CRLF).append(optionalParameters).append(CRLF).flush();
            ...
            writer.append("--" + boundary).append(CRLF);
            writer.append("Content-Disposition: form-data; name=\"uploadFile\"; filename=\"" + fileName + "\"")
                    .append(CRLF);
    ...
    public String uploadFile(String documentId, String revision, String changeReason, String checkInDocumentState,
        ...
        return uploadFile(documentId, revision, changeReason, checkInDocumentState, indexFields, fileName,
                filePath, mimeType, charset, "{}");
    }
    ...
    // the optional parameters argument.
    public String uploadFile(String documentId, String revision, String changeReason, String checkInDocumentState,
            String indexFields, String fileName, String filePath, String mimeType, String charset,
            String optionalParameters) throws Exception {
